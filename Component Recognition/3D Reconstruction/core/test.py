...
def test_net(cfg, epoch_idx=-1, output_dir=None, test_data_loader=None, \
    ...
    if decoder is None or encoder is None:
        ...
        # 新增代码
        encoder.load_state_dict({k.replace('module.',''):v for k,v in checkpoint['encoder_state_dict'].items()})
        decoder.load_state_dict({k.replace('module.',''):v for k,v in checkpoint['decoder_state_dict'].items()})

        if cfg.NETWORK.USE_REFINER:
            # refiner.load_state_dict(checkpoint['refiner_state_dict'])
            refiner.load_state_dict({k.replace('module.',''):v for k,v in checkpoint['refiner_state_dict'].items()})
        if cfg.NETWORK.USE_MERGER:
            # merger.load_state_dict(checkpoint['merger_state_dict'])
            merger.load_state_dict({k.replace('module.',''):v for k,v in checkpoint['merger_state_dict'].items()})
